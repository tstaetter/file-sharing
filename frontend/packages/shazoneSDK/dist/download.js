 ...
 */
function consumeBytes(pending, n) {
...
export async function downloadFile(apiPrefix, fileId, rawKey, onProgress) {
    ...
    if (pendingSize > 0) {
        if (pendingSize < IV_LEN + GCM_TAG_LEN + 1) {
            throw new Error(`Corrupt stream: remaining ${pendingSize} bytes are too short to form a valid encrypted chunk ` +
    ...
    const blob = new Blob(plaintextChunks, { type: contentType });
    onProgress?.(1);
